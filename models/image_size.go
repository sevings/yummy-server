// Code generated by go-swagger; DO NOT EDIT.

package models

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"context"

	"github.com/go-openapi/strfmt"
	"github.com/go-openapi/swag"
)

// ImageSize image size
//
// swagger:model ImageSize
type ImageSize struct {

	// height
	Height int64 `json:"height,omitempty"`

	// preview
	Preview string `json:"preview,omitempty"`

	// url
	URL string `json:"url,omitempty"`

	// width
	Width int64 `json:"width,omitempty"`
}

// Validate validates this image size
func (m *ImageSize) Validate(formats strfmt.Registry) error {
	return nil
}

// ContextValidate validates this image size based on context it is used
func (m *ImageSize) ContextValidate(ctx context.Context, formats strfmt.Registry) error {
	return nil
}

// MarshalBinary interface implementation
func (m *ImageSize) MarshalBinary() ([]byte, error) {
	if m == nil {
		return nil, nil
	}
	return swag.WriteJSON(m)
}

// UnmarshalBinary interface implementation
func (m *ImageSize) UnmarshalBinary(b []byte) error {
	var res ImageSize
	if err := swag.ReadJSON(b, &res); err != nil {
		return err
	}
	*m = res
	return nil
}
