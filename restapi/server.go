...
// NewServer creates a new api mindwell server but does not configure it
func NewServer(api *operations.MindwellAPI) *Server {
	s := new(Server)
...
type Server struct {
	...
	SocketPath    flags.Filename `long:"socket-path" description:"the unix socket to listen on" default:"/var/run/mindwell.sock"`
...
func (s *Server) SetAPI(api *operations.MindwellAPI) {
	if api == nil {
		s.api = nil
		s.handler = nil
		return
